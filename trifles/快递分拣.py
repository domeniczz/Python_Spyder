...
f = open("my_deliver_data.txt", mode="r", encoding='UTF-8')
tmp = f.read().split('\n')
# print(len(tmp))
for index in range(0, len(tmp), 2):
    tmp_list = [tmp[index], tmp[index + 1]]
...
for i in data_list:
    ...
    # print(i[1][:3])
    if '省' in i[1][:3]:
        place_name = i[1][0:i[1].index('省') + 1]
    elif '市' in i[1][:3]:
        place_name = i[1][0:i[1].index('市') + 1]
    elif '新疆' in i[1][:3]:
        place_name = '新疆维吾尔自治区'
    elif '内蒙' in i[1][:3]:
        place_name = '内蒙古自治区'
    ...
    # 判断省或者市是不是存在在字典里
    # 否，则新建，是，就添加
    if place_name not in new_dict:
        new_dict[place_name] = []
        new_dict[place_name].append(i)
    else:
        new_dict[place_name].append(i)

# print(new_dict)
print('{')
for k in new_dict:  # 选出每个省、直辖市、自治区
    print('\t' + k + ':[')
    for i in new_dict[k]:
        print('\t\t' + str(i))  # 选出具体内容
    print('\t],')
